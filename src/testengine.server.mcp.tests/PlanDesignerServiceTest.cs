...
namespace testengine.server.mcp.tests
{
    public class PlanDesignerServiceTest
    {
        ...
        public void GetPlanDetails_ShouldReturnPlanDetails_WhenPlanExists()
        {
            ...
            _mockSourceCodeService.Setup(m => m.LoadSolutionFromSourceControl(solutionId.ToString(), string.Empty)).Returns(null);

            // Act
            var planDetails = _planDesignerService.GetPlanDetails(planId);
