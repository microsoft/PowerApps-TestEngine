...
{
    /// <summary>
    /// This class provides direct access helper methods to create SaveFactFunction and ExportFactsFunction
    /// for testing without having to directly reference the implementations
    /// </summary>
    public static class ScanStateManagerAccess
    {
        /// <summary>
        /// Creates a new instance of the AddFactFunction
        /// </summary>
        public static AddFactFunction CreateAddFactFunction(RecalcEngine recalcEngine)
        {
            return new AddFactFunction(recalcEngine);
        }
    ...
    /// <summary>
    /// Test-specific implementation of SaveFactFunction that has the same interface as ScanStateManager.SaveFactFunction
    /// </summary>
    public class SaveFactFunctionForTests
    {
        ...
        public SaveFactFunctionForTests(IFileSystem fileSystem, ILogger logger, string workspacePath)
        {
    ...
    /// <summary>
    /// Test-specific implementation of ExportFactsFunction that has the same interface as ScanStateManager.ExportFactsFunction
    /// </summary>
    public class ExportFactsFunctionForTests
    {
        private readonly IFileSystem _fileSystem;
        private readonly ILogger _logger;
        private readonly string _workspacePath;
        ...
        public ExportFactsFunctionForTests(IFileSystem fileSystem, ILogger logger, string workspacePath)
        {
            _fileSystem = fileSystem ?? throw new ArgumentNullException(nameof(fileSystem));
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
            _workspacePath = workspacePath ?? throw new ArgumentNullException(nameof(workspacePath));
        ...
        public BooleanValue Execute(RecordValue parameters)
        {
            try
            {
                ...
                if (appPathValue is StringValue stringAppPathValue)
                {
                    ...
                    // Add recommendations
                    appFacts["TestRecommendations"] = GenerateTestRecommendations(appFacts);                    // Write to file
