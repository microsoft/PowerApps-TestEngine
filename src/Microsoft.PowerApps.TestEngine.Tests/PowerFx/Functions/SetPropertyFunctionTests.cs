...
namespace Microsoft.PowerApps.TestEngine.Tests.PowerFx.Functions
{
    public class SetPropertyFunctionTests
    {
        ...
        public void SetPropertyFunctionTest()
        {
            ...
            MockPowerAppFunctions.Verify(x => x.SetPropertyAsync(It.Is<ItemPath>((item) => item.ControlName == recordValue.Name), It.Is<StringValue>((stringVal) => stringVal == StringValue.New("5"))), Times.Once());
            ...
            Assert.IsType<BlankValue>(result);
            MockPowerAppFunctions.Verify(x => x.SetPropertyAsync(It.Is<ItemPath>((item) => item.ControlName == recordValue.Name), It.Is<StringValue>((stringVal) => stringVal == StringValue.New("10"))), Times.Once());
        }
    }
}
