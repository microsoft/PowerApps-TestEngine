...
namespace Microsoft.PowerApps.TestEngine.Tests.PowerApps
{
    public class PowerAppFunctionsTest
    {
        ...
        public async Task SetPropertyDateAsyncTest()
        {
            ...
            Assert.Equal(true, result);
            MockTestInfraFunctions.Verify(x => x.RunJavascriptAsync<bool>($"setPropertyValue({itemPathString},{{\"propertyName\":Date.parse(\"{((DateValue)DateValue.NewDateOnly(dt.Date)).Value}\")}})"), Times.Once());
        }
        ...
        public async Task SetPropertyRecordAsyncTest()
        {
            ...
            var value = "{\"Value\":\"2\"}";
            Assert.Equal(true, result);
            MockTestInfraFunctions.Verify(x => x.RunJavascriptAsync<bool>($"setPropertyValue({itemPathString},{{\"propertyName\":{value}}})"), Times.Once());
        }
        ...
        public async Task SetPropertyTableAsyncTest()
        {
            ...
            var itemPath = JsonConvert.DeserializeObject<ItemPath>(itemPathString);
            var controlType = new RecordType().Add("Value", FormulaType.String);
