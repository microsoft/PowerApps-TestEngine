...
function getAppStatus() {
    if (UCWorkBlockTracker.isAppIdle()) {
        if (!getContainer().AppMagic.AuthoringTool.Runtime.existsOngoingAsync()) {
            return "Idle";
        }
    }
    return "Busy";
...
function buildObjectModel() {
    var controls = buildControlObjectModel();
    var customPageControl = {
        name: 'CustomPage1',
        properties: [
        ]
    };
    controls.forEach(customPageChildControl => {
        customPageControl.properties.push({
            propertyName: customPageChildControl.name,
            propertyType: `![${customPageChildControl.name}: v]`
        })
    })
    controls.push(customPageControl);
    return {
        controls: controls
    }
}
...
function getPropertyValue(itemPath) {
    return getPropertyValueFromControl(itemPath)
}
...
// modified copy of PublishedAppTesting.js
...
function fetchArrayItemCount(itemPath) {
    ...
    var replicatedContexts = getContainer().AppMagic.Controls.GlobalContextManager.bindingContext.replicatedContexts
...
function getBindingContext(itemPath) {
    var bindingContext = getContainer().AppMagic.Controls.GlobalContextManager.bindingContext;
    ...
    var componentBindingContext = getContainer().AppMagic.Controls.GlobalContextManager.bindingContext.componentBindingContexts.lookup(itemPath.controlName);
...
function selectControl(itemPath) {
    var screenId = getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(getContainer().AppMagic.AuthoringTool.Runtime.getCurrentScreenName()).OpenAjax.uniqueId;
    ...
    if (itemPath.parentControl && itemPath.parentControl.index !== null) {
        ...
        if (itemPath.parentControl.parentControl) {
            ...
            return getContainer().AppMagic.Functions.select(null,
                bindingContext,
                getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(itemPath.parentControl.controlName),
                rowOrColumnNumber,
                getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(itemPath.controlName).OpenAjax._icontrol,
                screenId)
        }
    }
    ...
    if (itemPath.parentControl) {
        // Component
        var bindingContext = getContainer().AppMagic.Controls.GlobalContextManager.bindingContext.componentBindingContexts.lookup(itemPath.parentControl.controlName);
    ...
    return getContainer().AppMagic.Functions.select(null,
        getContainer().AppMagic.Controls.GlobalContextManager.bindingContext,
        getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(itemPath.controlName),
        null,
        null,
        screenId);
}
...
function setPropertyValueForControl(itemPath, value) {
    ...
    if (obj.parentControl && obj.parentControl.index !== null) {
        ...
        return getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(obj.controlName, galleryBindingContext).OpenAjax.setPropertyValueInternal(obj.propertyName, val, galleryBindingContext)
    }

    if (obj.parentControl) {
        // Component
        var componentBindingContext = getContainer().AppMagic.Controls.GlobalContextManager.bindingContext.componentBindingContexts.lookup(obj.parentControl.controlName);
        return (getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(obj.controlName, componentBindingContext).OpenAjax.setPropertyValueInternal(obj.propertyName, val, componentBindingContext));
    }

    return getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(obj.controlName, getContainer().AppMagic.Controls.GlobalContextManager.bindingContext).OpenAjax.setPropertyValueInternal(obj.propertyName, val, getContainer().AppMagic.Controls.GlobalContextManager.bindingContext);
}

function interactWithControl(itemPath, value) {
    var e = getContainer().AppMagic.AuthoringTool.Runtime.getNamedControl(itemPath.controlName, getContainer().AppMagic.Controls.GlobalContextManager.bindingContext).OpenAjax.interactWithControlAsync(getContainer().AppMagic.Controls.GlobalContextManager.bindingContext.controlContexts[itemPath.controlName], value).then(() => true, () => false);
