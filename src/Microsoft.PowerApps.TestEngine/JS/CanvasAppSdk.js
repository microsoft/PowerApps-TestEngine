...
class TestEnginePlugin {
  ...
  processResult(commandInfo) {
    if (commandInfo.args[1].successful) {
      callbackDictionary[commandInfo.args[0]].complete(
        commandInfo.args[1].message
      );
    } else {
      callbackDictionary[commandInfo.args[0]].error(
        commandInfo.args[1].message
      );
    }
...
function executePublishedAppScript(scriptToExecute) {
  ...
  AppMagic.Runtime.WebPlayerRuntime._appHostManager._apiHandler
    .invokeScriptAsync(`
        try {
...
function getOngoingActionsInPublishedApp() {
  return executePublishedAppScript(
    "AppMagic.AuthoringTool.Runtime.existsOngoingAsync()"
  );
}
...
function setPropertyValueForControl(itemPath, value) {
  var script = `setPropertyValueForControl(${JSON.stringify(
    itemPath
  )}, ${value})`;
  return executePublishedAppScript(script);
}
...
function getAppStatus() {
  if (
    typeof AppMagic === "undefined" ||
    typeof AppMagic.Runtime === "undefined" ||
    typeof AppMagic.Runtime.WebPlayerRuntime === "undefined" ||
    typeof AppMagic.Runtime.WebPlayerRuntime._appHostManager === "undefined"
  ) {
    return "Loading";
  }
  if (AppMagic.Runtime.WebPlayerRuntime._appHostManager._appIsLoading) {
    ...
    if (!isPluginRegistered) {
      AppMagic.Runtime.WebPlayerRuntime._appHostManager._apiHandler.registerHandler(
        testEnginePluginName,
        new TestEnginePlugin()
      );
    }
